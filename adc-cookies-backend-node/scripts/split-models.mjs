...
/*
 * drizzle-kit writes numeric defaults as STRINGS — money(...).default('0'). Our money type hands
 * back a number, so the two disagree, and TypeScript says so. It is the same string/number muddle
 * that produced "185" + "49" in the first place, one layer further out, so it is fixed rather than
 * cast away: a numeric column gets a numeric default.
 */
SRC = SRC.split('\n').map((line) =>
  line.includes('money(') ? line.replace(/\.default\('(-?\d+(?:\.\d+)?)'\)/g, '.default($1)') : line
).join('\n');
...
for (const [group, tables] of Object.entries(GROUPS)) {
  ...
  const imports = [
    ...
    ...[...needByFile.entries()].sort()
      /* .js in the specifier even though the file is .ts — that is what moduleResolution
         NodeNext wants, and what makes the emitted JavaScript correct too. */
  ...
  fs.writeFileSync(path.join(OUT, `${group}.model.ts`), `${header}${imports}\n\n${body}\n`);
  written++;
}
