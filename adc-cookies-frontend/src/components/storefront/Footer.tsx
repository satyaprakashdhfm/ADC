...
export default function Footer() {
  return (
    <footer style={{ background: 'var(--ink-950)', color: 'var(--cream-100)', padding: '40px 0 22px', borderTop: '1px solid var(--cream-100-14)' }}>
      <div className="footer-grid" style={{ maxWidth: 1180, margin: '0 auto', padding: '0 var(--gutter)', display: 'grid', gridTemplateColumns: '1.5fr repeat(3,1fr)', gap: 28 }}>
        <div>
          ...
          <p style={{ color: 'var(--cream-100-55)', maxWidth: 240, lineHeight: 1.6, fontSize: 'var(--text-xs)', margin: '0 0 14px' }}>Handcrafted cookies, baked fresh daily. Aroma of Freshness, delivered warm.</p>
  ...
  <svg key="x" width={18} height={18} viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.747l7.73-8.835L1.254 2.25H8.08l4.258 5.625 5.906-5.625Zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>,
              ...
              <span key={i} style={{ width: 40, height: 40, borderRadius: '50%', background: 'var(--white-08)', display: 'grid', placeItems: 'center', cursor: 'pointer', color: 'var(--cream-100-60)' }}>{Icon}</span>
            ))}
          </div>
        </div>
        <FooterCookies />
        {([
          ...
          <div key={h}>
            <div style={{ fontWeight: 700, color: 'var(--white)', marginBottom: 10, fontSize: 'var(--text-xs)', letterSpacing: '.08em', textTransform: 'uppercase' }}>{h}</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {items.map(([it, href]) => (
                <Link key={it} href={href} style={{ color: 'var(--cream-100-50)', textDecoration: 'none', fontSize: 'var(--text-sm)' }}>{it}</Link>
              ))}
            </div>
          </div>
        ))}
      ...
      <div style={{
        maxWidth: 1180, margin: '26px auto 0', padding: '16px var(--gutter) 0',
        borderTop: '1px solid var(--white-10)',
        display: 'flex', justifyContent: 'space-between',
        color: 'var(--cream-100-30)', fontSize: 'var(--text-xs)', flexWrap: 'wrap', gap: 8,
      }}>
