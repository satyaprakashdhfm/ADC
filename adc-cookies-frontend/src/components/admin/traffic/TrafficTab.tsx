'use client';
import { Users, UserPlus, FileText, Clock, Megaphone, Globe } from 'lucide-react';
...
import { Section, Notice, GroupHeading } from './TrafficParts';
...
/**
 * The admin "Traffic & ads" tab: what the ads cost and brought in, then everyone else who visited
 * and where from, in words the shop uses.
 *
 * Two halves under their own headings, "Ads tracking" and "Other traffic tracking", both scoped to
 * the one date filter at the top. "On the site right now" opens the traffic half; it ignores the
 * dates and says so in its title.
 */
export default function TrafficTab({ report, live, range, setRange, error, refreshing, refresh }: Props) {
  ...
  /* Each half waits on the same report. The error itself is shown once, above both. */
  const whenLoaded = (render: (report: AdminTraffic, g: AdminTraffic['google']) => React.ReactNode) =>
    error ? <Empty text="Could not load. See the note above." />
      : loading || !report || !g ? <Empty text="Loading…" />
      : render(report, g);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <GroupHeading icon={<Megaphone size={20} />} title="Ads tracking"
        hint="Our Meta ads that link to the website: what they cost, how many people they sent, and what those people ordered." />
      {whenLoaded(report => <MetaAdsSection report={report} />)}

      <GroupHeading icon={<Globe size={20} />} title="Other traffic tracking"
        hint="Everyone who opened the site, whatever brought them: Google, Instagram, WhatsApp, an ad, or typing the address." />
      <LiveNow live={live} />
      {whenLoaded((report, g) => (
        <>
